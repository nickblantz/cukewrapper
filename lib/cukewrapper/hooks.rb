...
module Cukewrapper
  ...
  module Hooks
    ...
    def self.register(name, event, priority: DEFAULT_PRIORITY, &block)
      ...
      Cukewrapper.log.debug("#{self.class.name}\##{__method__}") { "Registering hook for #{event}" }
    ...
    def self.trigger(event, *args)
      ...
      Cukewrapper.log.debug("#{self.class.name}\##{__method__}") { "Triggering hooks for #{event}" }
